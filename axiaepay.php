...
//
// 	Copyright (c) 2001-2013 Axia ePay
//	For assistance please contact devsupport@axiaepay.com
//

define("AXIAEPAY_VERSION", "1.6.13");


/**
 * Axia ePay Transaction Class
 *
 */
class umTransaction {
	...
	function umTransaction()
	{
		...
		$this->software="Axia ePay PHP API v" . AXIAEPAY_VERSION;
	...
	/**
	 * Send transaction to the Axia ePay Gateway and parse response
	 *
	 * @return boolean
	 */
	function Process()
	{
		...
		// Figure out URL
		$url = ($this->gatewayurl?$this->gatewayurl:"https://secure.axiaepay.com/gate");
		if($this->usesandbox) $url = "https://sandbox.axiaepay.com/gate";
	...
	function ProcessQuickSale()
	{
		...
		$data = '<?xml version="1.0" encoding="UTF-8"?>' .
				'<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ns1="urn:axiaepay" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:SOAP-ENC="http://schemas.xmlsoap.org/soap/encoding/" SOAP-ENV:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">' . 
				'<SOAP-ENV:Body>' .
				'<ns1:runQuickSale>' .
		...
		// Figure out URL
		$url = ($this->gatewayurl?$this->gatewayurl:"https://secure.axiaepay.com/soap/gate/15E7FB61");
		if($this->usesandbox) $url = "https://sandbox.axiaepay.com/soap/gate/15E7FB61";
	...
	function getSession()
	{
		...
		// Figure out URL
		$url = 'https://secure.axiaepay.com/interface/profiler/';
		if(preg_match('~https://([^/]*)/~i', $this->gatewayurl, $m)) {
			$url = 'https://' . $m[1] . '/interface/profiler/';
		}
		
		// Look for usesandbox
		if($this->usesandbox) $url = "https://sandbox.axiaepay.com/interface/profiler/";
	...
	/**
	 * Verify Proper Installation of Axia ePay class and required PHP Support
	 *
	 */
	function Test()
	{
		...
		<td valign="top"><?php
			if($curl_ssl_version) {	
				...
				Proper SSL installation is required to communicate with the AxiaePay gateway securely.  Please recompile curl
				with SSL support.
				<?php
			}
		?>					
		...
		<td valign="top"><?php
			...
			if(!$curl_version) {
				...
				$ch = curl_init(($this->gatewayurl?$this->gatewayurl:"https://secure.axiaepay.com/secure/gate.php") . "?VersionCheck=1&UMsoftware=" . rawurlencode($this->software));
				if(!$ch) {
					?><font color="red">Error</font><br>
					...
					if($return["UMversion"]) {
						$ssl_failed=false;
						?><font color="green">Ok</font><br>
						Successfully connected to the gateway.  Detected version (<?php echo $return["UMversion"]?>) of the Axia ePay gateway API.
						<?php						
					} else {
						$ch = curl_init(($this->gatewayurl?$this->gatewayurl:"https://secure.axiaepay.com/secure/gate.php") . "?VersionCheck=1&UMsoftware=" . rawurlencode($this->software));
						if($this->cabundle) curl_setopt($ch, CURLOPT_CAINFO, $this->cabundle);				
						...
						if($return["UMversion"]) {
							...
							if($return["UMversion"]) {
								...
								Unable to establish connection to the Axia ePay Gateway.  It is possible that you are firewalling
								outbound traffic from your server.  To correct this problem,  you must add an allow entry in
								your firewall for ip <?php echo gethostbyname("secure.axiaepay.com")?> on port 443;
								<?php								
		...
		<td valign="top"><?php
			...
			if(is_readable($certpath)) {	
				...
				<a href="https://secure.axiaepay.com/topics/curl-ca-bundle.crt">https://secure.axiaepay.com/topics/curl-ca-bundle.crt</a>				
				</tt>  
				<?php
			} else {
				?><font color="red">Error</font><br>
	...
	function httpPostPHP($url, $data)
	{
		...
		$options = array(
			'http'=> array(
	            ...
	            'user_agent' => 'uePHPLibary v' . AXIAEPAY_VERSION . ($this->software?'/' . $this->software:'')
	        ),
	        'ssl' => array(
